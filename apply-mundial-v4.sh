...
# apply-mundial-v4.sh — cambia las banderas de país de emoji de texto a
# imágenes SVG reales (librería flag-icons), porque muchos Windows/Chrome
# no dibujan el emoji de bandera y muestran el código de país en dos
# cuadraditos en su lugar. También añade la bandera que faltaba en
# "Mi equipo" y en los corredores revelados de "Clasificación".
#
# IMPORTANTE: este script SÍ trae package.json y package-lock.json
# actualizados (añade la dependencia flag-icons) — no hace falta que
# instales nada a mano, Vercel lo instalará solo al desplegar. No hay
# ningún .sql en este incremento.
#
# Ejecuta esto DESDE LA RAÍZ del repo (donde está db/schema.sql), con el
# Codespace ya abierto. Requiere haber aplicado antes apply-mundial-v3.sh.
...
echo "Aplicando el cambio de banderas a imágenes reales (v4)..."

echo "  - package.json"
cat > "package.json" <<'UKT_MUNDIAL_V4_EOF'
{
  "name": "web",
  "version": "0.1.0",
  "private": true,
...
cat > "package-lock.json" <<'UKT_MUNDIAL_V4_EOF'
{
  "name": "web",
  "version": "0.1.0",
  "lockfileVersion": 3,
...
mkdir -p "src/app"
cat > "src/app/globals.css" <<'UKT_MUNDIAL_V4_EOF'
@import "tailwindcss";
...
mkdir -p "src/lib"
cat > "src/lib/mundial.ts" <<'UKT_MUNDIAL_V4_EOF'
import { sql } from "./db";
...
echo "  - src/components/country-flag.tsx"
mkdir -p "src/components"
cat > "src/components/country-flag.tsx" <<'UKT_MUNDIAL_V4_EOF'
import { countryIso } from "@/lib/mundial";
...
mkdir -p "src/components"
cat > "src/components/mundial-riders-manager.tsx" <<'UKT_MUNDIAL_V4_EOF'
"use client";
...
mkdir -p "src/components"
cat > "src/components/mundial-squad-selector.tsx" <<'UKT_MUNDIAL_V4_EOF'
"use client";
...
cat > "src/app/mundial/equipo/page.tsx" <<'UKT_MUNDIAL_V4_EOF'
...
cat > "src/app/mundial/clasificacion/page.tsx" <<'UKT_MUNDIAL_V4_EOF'
import { sql } from "@/lib/db";
import { getSession } from "@/lib/auth";
...
git add -A
git commit -m "Mundial: banderas de país como imagen real en vez de emoji

El emoji de bandera de texto no se dibuja en muchos Windows/Chrome (se ve
el código de dos letras en dos cuadraditos); se sustituye por un icono
SVG real (librería flag-icons) que se ve igual en cualquier sistema.
También añade la bandera, que faltaba, en Mi equipo y en los corredores
revelados de Clasificación."
git push

echo ""
echo "Listo. No hace falta ejecutar nada en Neon para este cambio."
echo "Vercel instalará la nueva dependencia (flag-icons) solo al desplegar."
