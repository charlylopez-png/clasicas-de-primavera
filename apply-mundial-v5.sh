...
# apply-mundial-v5.sh — "Mi equipo" pasa a mostrarse en 3 bloques de color
# (Amarillo/Rosa/Verde), cada uno con su cupo (1/2/3) bien visible, y con
# botones para añadir/quitar corredores de ese color sin salir de la
# pantalla. Si un equipo ya guardado deja de cumplir el cupo porque el
# admin recategorizó a uno de sus corredores después del fichaje, se avisa
# arriba del todo para que se pueda corregir ahí mismo.
#
# No hay ningún .sql en este incremento ni dependencias nuevas.
#
# Ejecuta esto DESDE LA RAÍZ del repo (donde está db/schema.sql), con el
# Codespace ya abierto. Requiere haber aplicado antes apply-mundial-v4.sh.
...
echo "Aplicando el editor de Mi equipo por bloques de color (v5)..."

echo "  - src/components/mundial-team-editor.tsx"
mkdir -p "src/components"
cat > "src/components/mundial-team-editor.tsx" <<'UKT_MUNDIAL_V5_EOF'
"use client";
...
cat > "src/app/mundial/equipo/page.tsx" <<'UKT_MUNDIAL_V5_EOF'
...
git add -A
git commit -m "Mundial: Mi equipo en bloques de color, con aviso de cupo roto

'Mi equipo' pasa de ser un resumen fijo a un editor con 3 bloques
(Amarillo/Rosa/Verde), cada uno mostrando su cupo (1/2/3) y con botones
para añadir/quitar corredores de ese color. Si un equipo ya guardado deja
de cumplir el cupo porque un corredor fichado fue recategorizado después,
se avisa arriba para poder corregirlo ahí mismo. El guardado sigue
validando en el servidor que el equipo sea exactamente 1 Amarillo + 2
Rosas + 3 Verdes."
git push
